...
export default function LeadSubmissionEmail(data: Props) {
  const { firstname, lastname, email, phone, productTypes, quantity, material, projectType } = data;
      ...
      <Tailwind>
        <Container style={{ backgroundColor: "#ffffff" }} className="bg-white p-6 rounded-lg shadow-lg border border-gray-200 max-w-xl mx-auto">
          ...
          <Section style={{ backgroundColor: "#f3f4f6", padding: "20px", textAlign: "center", borderRadius: "8px", marginBottom: "20px" }}>
            ...
            <Text><strong>Produit :</strong> {productTypes.length > 0 ? productTypes.join(", ") : "Non renseigné"}</Text>
            <Text><strong>Quantité :</strong> {quantity}</Text>
            <Text><strong>Matériau :</strong> {material}</Text>
            <Text><strong>Type de projet :</strong> {projectType}</Text>
          </Section>
